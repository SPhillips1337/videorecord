	...
	if (localStream == null) {
		...
		mediaRecorder.onstop = function(){
			...
			*/
			...
			/* lets post our video data to be transcoded */
			...
			fetch('transcode.php?rand='+rand, {
				method: 'POST',
				body: formData
				})
				.then(response => {console.log('Transcription complete');})
				.then(() => {
					$('#transcription').val(response.text());
				})
		   		.catch(error => {console.log('error');});


			/*
			// Send video data to PHP script
			$.post('transcode.php', {videoData: this.recording}, function(textData){
				// Output text data to HTML5 video tag
				$('#transcription').val(textData);
			});*/
